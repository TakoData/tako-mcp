...
- Empty result (zero cards): do NOT reword and retry blind. Every search is billed. Recover in exactly this order: (1) `tako_available_data` (free) for the exact metric name + `node_id`; (2) if it reports coverage, ONE more search with that exact name, pinning the METRIC `node_id` alone and setting `strict: true` (an entity-only pin, or a pin without `strict`, does not steer retrieval); (3) if not covered, stop searching Tako and answer from the web results already in the response. Never send more than 2 priced searches for the same underlying question (in a fan-out, each entity+metric query has its own budget).
...
- Empty result (zero cards): do NOT reword and retry blind; every search is billed. Recover in order: (1) `tako_available_data` (free) for the exact indicator name + `node_id`; (2) if covered, ONE more search with that exact name, pinning the METRIC `node_id` alone and setting `strict: true` (an entity-only pin, or a pin without `strict`, does not steer retrieval); (3) if not covered, stop searching Tako and answer from the web results already in the response. Never more than 2 priced searches per underlying question (in a fan-out, each country+indicator query has its own budget).
