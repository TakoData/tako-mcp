...
import { FREE_TIER_TOOL_NAMES } from "../src/freetier.js";
import {
...
export function buildToolsDoc(input: ToolsDocInput): string {
  ...
  const out: string[] = [];
  out.push(
    `<!-- GENERATED by workers/scripts/gen-registry.ts from workers/src/tools/*.ts (tako-mcp ${input.version}). Do not edit; run npm run registry:gen in workers/. -->`,
    "",
    "# Tako MCP tools",
    "",
    ...
    "",
    "On `/mcp`, `?tools=` on the connection URL is an allowlist that **replaces** the default listing: `?tools=search,contents` lists exactly those two. Tokens are tool names; the `tako_` prefix is optional. Unknown tokens are dropped, and a param that names nothing recognisable yields the defaults, so a typo never breaks a connection. If you list tools, include the defaults you rely on — descriptions assume `tako_search`, `tako_available_data`, and `tako_contents` are present. `/mcp/chatgpt` ignores the param: its listing is fixed at submission.",
    "",
  );
  ...
  for (const m of modules) {
    ...
    out.push(`- Runs anonymously: ${input.freeTierToolNames.has(m.name) ? "yes" : "no (answers with sign-in instructions)"}`, "");
