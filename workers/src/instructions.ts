...
import { DOMAIN_NAMES_INLINE } from "./vocabulary.js";
...
const SHARED_INSTRUCTION_PARAGRAPHS = [
  // The domain list is interpolated from `vocabulary.ts`, not written out here.
  // Point 1 above is right that it is load-bearing -- which is exactly why it
  // must have ONE definition: it was also written out in the coverage resource,
  // and the two had already diverged. This listed `weather`, the resource did
  // not, and the resource listed `US government spending` that this did not, so
  // an agent reading the instructions and the resource in one session was told
  // both that Tako has weather data and that it does not. Interpolating a
  // complete enumeration after a colon substitutes a self-contained block, not a
  // vocabulary term spliced mid-sentence.
