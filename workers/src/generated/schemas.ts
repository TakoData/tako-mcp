...
export const GraphRelationType = z.enum(["metric","entity","member"]);
...
export const RelatedGroups = z.object({ "metrics": z.lazy(() => RelationGroup), "entities": z.lazy(() => RelationGroup), "part_of": z.lazy(() => RelationGroup), "members": z.lazy(() => RelationGroup) });
export type RelatedGroups = z.infer<typeof RelatedGroups>;

export const RelationGroup = z.object({ "items": z.array(z.lazy(() => GraphNode)), "total": z.number().int(), "total_capped": z.boolean().describe("True when `total` hit the server-side counting cap; the true count is at least `total`. Render as '>total'.").default(false) });
