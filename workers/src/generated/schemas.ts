...
export const AgentUsage = z.object({ "usd_cost": z.union([z.number(), z.null()]).optional(), "compute_usd": z.union([z.number(), z.null()]).optional(), "data_output_usd": z.union([z.number(), z.null()]).optional(), "data_output_datasets": z.union([z.number().int(), z.null()]).optional() }).describe("Cost-plus usage for an agent run. usd_cost is the TOTAL charged (compute +\ndata-output surcharge) — its meaning is unchanged from v1. The breakdown is\nadditive (TAKO-3376): compute_usd = COGS x AGENT_COST_MULTIPLIER;\ndata_output_usd = the per-source data-output surcharge (0 today);\ndata_output_datasets = the billing quantity (distinct finalized, data-bearing\ndatasets charged — S6a/TAKO-3406; was distinct filled slots pre-S6a).\nPresent only on metered/PAYG runs that were actually billed; absent otherwise.\nNo token/tool-call/latency telemetry.");
