...
const create_report = {
  name: "create_report",
  description:
    "Use this when the user asks to generate a Tako report on a topic (e.g. \"write a Tesla Q1 earnings report\"). Kicks off async generation and returns report_id + webpage_url.\n\n**HARD RULE — the Tako report IS the answer.** Do NOT write the report content yourself. Do NOT call `web_search`, `browse`, or any other research tool while the report runs. Do NOT summarize, paraphrase, pre-empt, or improvise an answer from your own knowledge. Do NOT produce analysis, data, charts, tables, or commentary about the topic before `wait_for_report` returns a terminal status. Your job between create_report and the terminal wait_for_report is exactly two things: (1) share the `webpage_url` link and tell the user the report is being generated, (2) keep calling `wait_for_report` until done. Anything else is wrong.\n\n**Flow:** IMMEDIATELY after this returns, share the `webpage_url` as a clickable link AND tell them they'll get an email when ready, then call `wait_for_report(report_id)` and continue chaining further `wait_for_report` calls back-to-back until status is 'completed' or 'failed' — reports take 5–20 minutes, so expect 5–12 chained calls. CAP THE CHAIN AT 12 CALLS TOTAL (~10 minutes): if 12 chained wait_for_report calls elapse without a terminal status, STOP chaining and tell the user the report is taking longer than usual; the email notification will reach them when it's done. Once status is terminal, summarize the report's actual `result` (or `error_message` on failed) and re-share the `webpage_url`. The summary MUST be drawn ONLY from the wait_for_report `result` field — never from your own knowledge or external sources.\n\nPass only `report_type`, `title`, and `research_objective` — the tool resolves the right template internally. Do NOT pre-call get_credit_balance; the response carries credit_cost.",
  inputSchema,
  outputSchema,
  annotations: {
