...
const knowledge_search = {
  ...
  description:
    "Use this to find existing charts and live-data visualizations on almost any topic. Tako's knowledge base covers economics, finance (stocks, crypto, FX), demographics, technology, weather and forecasts, polls and elections, prediction markets (Polymarket), internet and app traffic (SimilarWeb), sports, real-estate, energy, health, and more — plus real-time / live data via the deep-research pipeline. Default to calling this first whenever a user asks about any trend, comparison, statistic, current value, forecast, or betting/prediction-market odds, even if the topic seems outside traditional 'chart' categories — Tako very likely has a relevant card. After this tool returns, if the user asked to see / view / show / render / get a chart (e.g. \"give me a chart\", \"show AAPL\", \"chart the data\"), you MUST follow up immediately by calling `open_chart_ui` with `open_ui_args` from the top result — `open_chart_ui` is the tool that actually renders the chart inline in the client. Just writing \"Interactive chart:\" or pasting the URL does NOT render anything; the user sees no chart and has to ask a second time. Note: with `search_effort: \"deep\"` (and the default fast→deep auto-escalation when fast returns no cards) the call can run up to ~5 minutes server-side; MCP clients on the SDK default 60s timeout should pass `options.timeout` of at least 300_000 ms when invoking this tool, otherwise the client will abort before the deep result lands.",
  inputSchema,
  outputSchema,
  annotations: {
