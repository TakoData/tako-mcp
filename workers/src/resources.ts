...
function toolGuide(surface: Surface, tier: Tier): string {
  ...
  const sections = [
    ...
    "",
    "The table above is what this connection actually lists. `?tools=` on the",
    "endpoint REPLACES that listing with the names you pass rather than adding to",
    "it, so it narrows the surface — it does not unlock extras. See",
    "https://docs.tako.com.",
    "",
