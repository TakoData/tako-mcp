...
4. `tako_available_data` — See **what data Tako has** on an entity or metric: resolves the name to the top graph matches and summarizes each one's metrics in one free call (graph search → the metrics drill internally). Each match carries a `node_id` to pin into a follow-up `tako_search` / `tako_answer`.
