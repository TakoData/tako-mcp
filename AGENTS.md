...
2. `tako_contents` — Fetch what's behind a result url in full: a web page's text, or an exportable card's data rows as one projected `rows` shape. Four inputs — `urls` (required), `max_rows`, `max_chars`, `query`. Requires a signed-in connection. Default.
3. `tako_available_data` — Find **what proprietary, structured data exists** on an entity or metric, and confirm a specific figure exists (and its exact name) before spending a priced `tako_search`. Lists the available metrics in one free call. Each match carries its canonical name — search on that name, since `tako_search` takes no pin — plus an `id` for `tako_graph_related` traversal. Ambiguous `q` returns the top entity and top metric as candidates; `metric` is the substring browse filter; `limit` (max 20) widens the list. Default.
4. `tako_graph_related` — Explore a node's relations (metrics, entities, `rel:competes_with`, `part_of`, `members`, sources) as a map or one paged relation. The map previews each group's first names only; a drilled item carries `id`, `name` and `kind`. `q` is a substring filter. Drill into a node `tako_available_data` already resolved. Default.
5. `tako_search_advanced` — The WHOLE v3 `SearchRequest` body plus `include_answer` and `output_schema`, derived from `workers/src/generated/schemas.ts` so the set cannot drift (parity is tested, not asserted in a header). `include_answer: true` runs `POST /api/v1/answer` instead of `/api/v3/search`: one synthesized, citation-backed answer with the retrieval as its citations. Every level is `.strict()`, and `mcp.ts` registers the full schema object, so an unknown key is a -32602 on the wire. It is the only pin-capable tool, so the license-gated values path (METRIC node id alone + `strict: true` + `include_answer` + `data.include_contents`) lives here. Opt-in via `?tools=search_advanced`.
6. `tako_agent` — Answer Agent for multi-step data questions (~30–90s; the Worker polls the run to a terminal state inside the one tool call and returns the finished answer). Both channels carry the projected run: `answer`, `citations`, the `cards` it built, and the definitions/assumptions/methodology maps. NO progress notification reaches any host — the transport discards request-scoped notifications under `enableJsonResponse: true`, so the client's own tool timeout is the ceiling (TAKO-4485). Opt-in; off the chatgpt surface, whose ~60s per-call ceiling cannot hold the run.
7. `tako_visualize` — Create a card from your own structured data. The one tool here that WRITES: the card is public and permanent, and anyone with the returned url can open it without signing in. Opt-in on `/mcp`, listed by default on `/mcp/chatgpt`, the host that renders the widget inline.
