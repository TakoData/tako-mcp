...
3. `tako_available_data` — Find **what proprietary, structured data exists** on an entity or metric, and confirm a specific figure exists (and its exact name) before spending a priced `tako_search`. Summarizes the available metrics in one free call. Each match carries a `node_id` to pin into a follow-up `tako_search`. Ambiguous `q` returns the top entity and top metric as candidates; `metric` is the substring browse filter; `limit` (max 20) widens the list. Default.
