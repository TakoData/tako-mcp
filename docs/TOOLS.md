<!-- GENERATED by workers/scripts/gen-registry.ts from workers/src/tools/*.ts (tako-mcp 0.22.2). Do not edit; run npm run registry:gen in workers/. -->
...
- `tako_search_advanced` — `?tools=search_advanced`
...
- Runs anonymously: no (answers with sign-in instructions)
